...
public class Clubgoer extends Thread {
	...
	protected void moveRandomly() throws InterruptedException {
		int x_mv = rand.nextInt(3) - 1;
		int y_mv = rand.nextInt(3) - 1;
		currentBlock = club.move(currentBlock, x_mv, y_mv, myLocation);
		System.out.println("Thread " + this.ID + " moved randomly to position: " + currentBlock.getX() + " " + currentBlock.getY());
		sleep(movingSpeed);
	}
	...
	private void checkPause() throws InterruptedException{//checks if simulation is running or not
		synchronized (ClubSimulation.Paused){//controls the access to the Paused resource so it cant be changed or accessed by many threads at once
