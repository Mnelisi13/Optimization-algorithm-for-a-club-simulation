...
public class ClubSimulation {
	...
	private static int maxWait = 1000; //for the slowest customer
	//static ClubView barpersonLocation;
	private static int minWait = 200; //for the fastest cutomer
	private static final AtomicBoolean Resuming = new AtomicBoolean(false);
	...
	public static void setupGUI(int frameX, int frameY, int[] exits) {
		...
		startB.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				synchronized (Resuming) {
					if (!Resuming.get()) {
						Resuming.set(true);//signal simulation to start
						Resuming.notifyAll();
					}
				}
			}
		...
		pauseB.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				synchronized (Paused) {
					Paused.set(!Paused.get());//toggle the pause state
					if (Paused.get()) {
						pauseB.setText("Resume");//Change it to resume if simulation is in pause state
					} else {
						pauseB.setText("Pause");//that means its already running then it should be showing an option to pause
						Paused.notifyAll();
					}
				}
			}
	...
	public static boolean isPaused() {
		return Paused.get();//Checks if simulation is running or not by controlling a boolean variable.
	}
}
