...
public class ClubGrid {
	...
	private boolean inBarArea(int i) {
		if ((i>=x) || (i>bar_y) ||(i<0))
			return false;
		return true;
	}
	...
	public GridBlock enterClub(PeopleLocation myLocation) throws InterruptedException {
		synchronized (counter) {//Allow only one thread to access the entry at a time so the count increases incrementally by one.
			counter.personArrived();//update it when a person arrives
			...
			counter.notifyAll();//notify all the other counters
		}
		return entrance;
	}
	...
	public void leaveClub(GridBlock currentBlock, PeopleLocation myLocation) {
		synchronized (currentBlock) {//synchronize the way the threads access or customers use the exit(block)
			currentBlock.release();
			counter.personLeft(); // Subtract from counter
			myLocation.setInRoom(false);
			currentBlock.notifyAll(); // Notify waiting threads about the available space

			synchronized (counter) {
				counter.notifyAll(); // Notify waiting threads about the updated waiting count
			}
		}
	}
